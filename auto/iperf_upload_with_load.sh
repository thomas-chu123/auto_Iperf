...
if [ $1 -eq 0 ]; then
  iperf3 -c ${server_ip[0]} -B ${sfu_ip[0]} -i 5 -p ${port[0]} -f m -t 65 -l 1460 -P 4 -R &
  port_id="1"
  file=$3"_p"$port_id".txt"
  echo $file
  iperf3 -c ${server_ip[4]} -B ${sfu_ip[0]} -i 5 -p ${port[4]} -f m -t 60 -l 1460 -P 10 > ${file} &
  sleep 2
elif [ $1 -eq 1 ]; then
  iperf3 -c ${server_ip[4]} -B ${hb3_ip[0]} -i 5 -p ${port[4]} -f m -t 65 -l 1460 -P 2 -R &
  sleep 2
  iperf3 -c ${server_ip[5]} -B ${hb3_ip[1]} -i 5 -p ${port[5]} -f m -t 65 -l 1460 -P 2 -R &
  sleep 2
  for (( i=0; i<$2; i++ ))
  do
    port_id=$((i+1))
